import { Cell, Colors, Move, ReturnFromGetMovesFromPGN } from "../types.ts";
...
export class Game {
  ...
  public deducePieceOrigin(move: MoveNotation): Piece {
    ...
    const possiblePieces = this._board.getPiecesOfAKind(
      move.piece,
      this.currentPlayer
    );
    ...
    const validPieces = possiblePieces.filter((piece) =>
      piece.isValidMove(this._board, move)
    );
  ...
  public makeMove({ from, to, flags }: Move) {
    if (flags?.castle) {
      ...
      if (!piece) {
        ...
        const enPassantTargetSquare = this._board.getSquare(
          flags.enPassant.enPassantTargetSquare
        );
        ...
        const promotionPiece = new PieceClass(
          this.currentPlayer,
          to,
          flags.promotion.promotionPiece
        );
