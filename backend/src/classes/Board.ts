...
import { nameClassRelation } from "../utils.ts";
...
const initialPiecePositions: Record<
  Colors,
  Record<PieceIdentifier, Array<Coordinates>>
> = {
  [Colors.WHITE]: {
...
export class Board {
  ...
  public getGrid() {
    return this.#grid.map((row) =>
      row.map((square) => square.piece?.pieceLetter || null)
    );
  }

  public get formatedGrid() {
    return this.#grid
      .map((row) => row.map(({ piece }) => piece?.pieceLetter || "-").join(" "))
      .join("\n");
  }
  ...
  public isTherePieceBetween(from: Coordinates, to: Coordinates) {
    ...
    const lengthWalked = Math.max(
      lengthWalkedVertically,
      lengthWalkedHorizontally
    );
    ...
    Object.entries(initialPiecePositions).forEach(([color, pieces]) => {
      Object.entries(pieces).forEach(([pieceType, positions]) => {
        positions.forEach((position) => {
          ...
          this.placePiece(
            new pieceClass(color as Colors, position, piece),
            position
          );
        });
      });
