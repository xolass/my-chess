...
export default class MoveNotation {
  isCapture: boolean;
  isCheck: boolean;
  isCheckmate: boolean;
  ...
  static toCoordinate(cell: Cell) {
    if (cell.length !== 2)
      throw new Error("Invalid cell notation", { cause: cell });
  ...
  private getTo(): Coordinates {
    ...
    if (this.getIsCheck() || this.getIsCheckmate()) {
      return MoveNotation.toCoordinate(this.move.slice(-3, -1));
    }
    if (this.promotion?.isPromotion) {
      return MoveNotation.toCoordinate(this.move.slice(-4, -2));
    }

    return MoveNotation.toCoordinate(this.move.slice(-2));
  }
  ...
  private getPromotion(): Promotion | undefined {
    if (this.move.includes("=")) {
      const promotionPiece = this.move
        .slice(-1)
        .toLowerCase() as PieceIdentifier;
